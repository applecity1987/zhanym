html = open('/Users/darknet/dating-site/templates/chat.html', 'w', encoding='utf-8')
html.write("""<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Чат — Zhanym</title>
<style>
...
<div class="chat-container">
  <div class="matches-list">
    <div class="matches-title">💞 Пары</div>
  ...
  <div class="chat-area">
    ...
    <div class="messages" id="messages">
      <div class="empty-chat">
        <h3>Выбери пару</h3>
        <p>Начни общение с понравившимся человеком</p>
      </div>
    </div>
...
function renderMatches(matches) {
  ...
  if(!matches || !matches.length){
    list.innerHTML = '<div class="no-matches">Пока нет пар.<br>Лайкай больше!</div>';
    return;
  }
  list.innerHTML = '';
  matches.forEach(function(m) {
    ...
    div.id = 'match-' + m.id;
    div.innerHTML = '<div class="match-avatar">' + (m.gender==='female' ? '👩' : '👨') + '</div><div><div style="font-weight:bold;font-size:14px">' + m.name + ', ' + m.age + '</div><div style="color:#aaa;font-size:12px">' + m.city + '</div></div>';
...
async function openChat(userId, name, gender) {
  ...
  document.getElementById('chatAvatar').textContent = gender === 'female' ? '👩' : '👨';
  if(ws) ws.close();
